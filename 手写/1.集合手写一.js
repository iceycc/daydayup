...
(function (done) {
    ...
    const match = curring(function (what, str) {
    ...
    // eg2 替换
    const replace = curring(function (what, replacement, str) {
    ...
    // 3 过滤
    const filter = curring(function (f, ary) {
        return ary.filter(f);
    });
    const f = filter((i) => i % 2 !== 0)
    console.log(f([1, 2, 3, 4, 5, 6, 7]))

    // 映射
    const map = curring(function (f, ary) {
        return ary.map(f);
    });
    let ma = map(i => i * 2);
    console.log(ma([1, 2, 3, 4, 5]))
})();
// 2- 实现 after 在执行多少次之后在执
(function (done) {
    if (!done) return;
...
(function (done) {
    ...
    // 中断
    function warpAbort(promise) {
        let abort
        let p2 = new Promise(function (resolve, reject) {
            abort = reject
        })
        let p3 = Promise.race(promise, p2)
        p3.abort = abort
        return p3
    }
...
(function (done) {
    ...
    Function.prototype.myBind = function (ctx, ...args) {
        ...
        const fnBound = function (...args2) {
            return self.apply(this instanceof fnBound ? this : ctx, args.concat(args2))
        };
        // fnBound.prototype = this.prototype
        Object.setPrototypeOf(fnBound.prototype, this.prototype) // 防止修改原来的prototype
        return fnBound
    };
    ...
    // obj.__proto__.friend = 'Tom' // 防止修改原来的prototype
    // console.log('----', (new bar('text', 18)).friend);
})();
// 12-new
(function (done) {
    if (!done) return
    ...
    function Person(name) {
        this.name = name
        return {
            habit: this.habit + ' hei',
            name: name + ' haha'
        }
    }

    Person.prototype.habit = 'game'
    ...
    console.log(p.name, p.habit)
    let p1 = objectFactory(Person, 'Tom')
    console.log(p1.name, p.habit)
})();
// 13 数组对象与arguments
(function (done) {
    if (!done) return
