 ...
 * Application类
 * 1. 创建路由实例 / 路由懒加载功能
 *  this.router = new Router()  / lazy_route
 * 2. 处理中间件逻辑,交给router实例去处理
 *  app.use => this.router.use(path,handler)
 * 3. 处理用户绑定路由（指定请求方法method = get/post/put等）
 *  app[method]  => this.router[method](path,handler)
 * 4. 开启服务，监听端口
 *  app.listen()
 *  处理请求来的方法
 *  const done = () => {/**请求不到，的处理函数}
 *  this.router.handler(req,res,done);
 * 5. 参数提前处理:对传入的key进行处理
 *  app.param  => this.router.param(key,handler);
 */
