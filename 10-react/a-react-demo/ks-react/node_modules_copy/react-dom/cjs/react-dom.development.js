...
var IS_EVENT_HANDLE_NON_MANAGED_NODE = 1 << 2;
var IS_NON_DELEGATED = 1 << 3;
var IS_CAPTURE_PHASE = 1 << 4;
var IS_REPLAYED = 1 << 6;
...
function SyntheticEvent(reactName, targetInst, nativeEvent, nativeEventTarget) {
  var Interface = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : EventInterface;
...
function extractCompositionEvent(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget) {
  ...
  var event = new SyntheticEvent(eventType, null, nativeEvent, nativeEventTarget, CompositionEventInterface);
...
function extractBeforeInputEvent(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget) {
  ...
  var event = new SyntheticEvent('onBeforeInput', null, nativeEvent, nativeEventTarget, InputEventInterface);
  accumulateTwoPhaseListeners(targetInst, dispatchQueue, event);
  event.data = chars;
}
/**
...
function checkControlledValueProps(tagName, props) {
  {
    if (!(hasReadOnlyValue[props.type] || props.onChange || props.onInput || props.readOnly || props.disabled || props.value == null || enableDeprecatedFlareAPI )) {
      error('You provided a `value` prop to a form field without an ' + '`onChange` handler. This will render a read-only field. If ' + 'the field should be mutable use `defaultValue`. Otherwise, ' + 'set either `onChange` or `readOnly`.');
    }

    if (!(props.onChange || props.readOnly || props.disabled || props.checked == null || enableDeprecatedFlareAPI )) {
...
function createAndAccumulateChangeEvent(dispatchQueue, inst, nativeEvent, target) {
  var event = new SyntheticEvent('onChange', null, nativeEvent, target);
...
function extractEvents$2(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  if (isOverEvent && (eventSystemFlags & IS_REPLAYED) === 0) {
    ...
    if (related) {
      ...
      var inst = getClosestInstanceFromNode(related);
  ...
  var leave = new SyntheticEvent(leaveEventType, from, nativeEvent, nativeEventTarget, eventInterface);
  leave.type = eventTypePrefix + 'leave';
  leave.target = fromNode;
  leave.relatedTarget = toNode;
  var enter = new SyntheticEvent(enterEventType, to, nativeEvent, nativeEventTarget, eventInterface);
  enter.type = eventTypePrefix + 'enter';
  enter.target = toNode;
  enter.relatedTarget = fromNode; // If we are not processing the first ancestor, then we
...
function constructSelectEvent(dispatchQueue, nativeEvent, nativeEventTarget) {
  ...
  if (!lastSelection || !shallowEqual(lastSelection, currentSelection)) {
    ...
    var syntheticEvent = new SyntheticEvent('onSelect', null, nativeEvent, nativeEventTarget);
...
function extractEvents$3(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  var eventListenerMap = getEventListenerMap(targetContainer); // Track whether all listeners exists for this plugin. If none exist, we do
  // not extract events. See #3639.

  if ( // If we are handling selectionchange, then we don't need to
  ...
  domEventName !== 'selectionchange' && !eventListenerMap.has('onSelect') && !eventListenerMap.has('onSelectCapture')) {
    return;
  }
...
function extractEvents$4(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  var event = new SyntheticEvent(reactName, null, nativeEvent, nativeEventTarget, EventInterface);
...

  {
    ...
    var accumulateTargetOnly = false;

    {
      accumulateTargetOnly = !inCapturePhase && // TODO: ideally, we'd eventually add all events from
...
function extractEvents$5(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  if (shouldProcessPolyfillPlugins) {
    ...
    extractEvents$3(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer);
...
function listenToNonDelegatedEvent(domEventName, targetElement) {
  ...
  var listenerMap = getEventListenerMap(targetElement);
  var listenerMapKey = getListenerMapKey(domEventName, isCapturePhaseListener);
  var listenerEntry = listenerMap.get(listenerMapKey);

  if (listenerEntry === undefined) {
    var listener = addTrappedEventListener(targetElement, domEventName, PLUGIN_EVENT_SYSTEM | IS_NON_DELEGATED, isCapturePhaseListener);
...
function listenToNativeEvent(domEventName, isCapturePhaseListener, rootContainerElement, targetElement, isPassiveListener, listenerPriority) {
  var eventSystemFlags = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : PLUGIN_EVENT_SYSTEM;
  ...
  var listenerMap = getEventListenerMap(target);
  var listenerMapKey = getListenerMapKey(domEventName, isCapturePhaseListener);
  var listenerEntry = listenerMap.get(listenerMapKey);
  ...
  if (listenerEntry === undefined || shouldUpgrade) {
    ...
    if (shouldUpgrade) {
      removeTrappedEventListener(target, domEventName, isCapturePhaseListener, listenerEntry.listener);
    }
    ...
    var listener = addTrappedEventListener(target, domEventName, eventSystemFlags, isCapturePhaseListener, false, isPassiveListener, listenerPriority);
    listenerMap.set(listenerMapKey, {
...
function listenToReactEvent(reactEvent, rootContainerElement, targetElement) {
  ...
  var dependenciesLength = dependencies.length; // If the dependencies length is 1, that means we're not using a polyfill
  ...
  var isPolyfillEventPlugin = dependenciesLength !== 1;

  if (isPolyfillEventPlugin) {
    var listenerMap = getEventListenerMap(rootContainerElement); // For optimization, we register plugins on the listener map, so we
    ...
    var isCapturePhaseListener = reactEvent.substr(-7) === 'Capture' && // Edge case: onGotPointerCapture and onLostPointerCapture
...
function addTrappedEventListener(targetContainer, domEventName, eventSystemFlags, isCapturePhaseListener, isDeferredListenerForLegacyFBSupport, isPassiveListener, listenerPriority) {
  var listener = createEventListenerWrapperWithPriority(targetContainer, domEventName, eventSystemFlags, listenerPriority); // If passive option is not supported, then the event will be
...
function accumulateSinglePhaseListeners(targetFiber, dispatchQueue, event, inCapturePhase, accumulateTargetOnly) {
  ...
  var targetType = event.type; // Accumulate all instances and listeners via the target -> root path.
...
function getListenerMapKey(domEventName, capture) {
...
var continuousReplayableEvents = ['dragenter', 'dragleave', 'focusin', 'focusout', 'mouseover', 'mouseout', 'pointerover', 'pointerout', 'gotpointercapture', 'lostpointercapture'];
function isReplayableDiscreteEvent(eventType) {
  return discreteReplayableEvents.indexOf(eventType) > -1;
}

function trapReplayableEventForContainer(domEventName, container) {
  listenToNativeEvent(domEventName, false, container, null);
}

function eagerlyTrapReplayableEvents(container, document) {
  var listenerMapForDoc = getEventListenerMap(document); // Discrete
    ...
    case DefaultLanePriority:
      {
        ...
        if (_lane3 === NoLane) {
          ...
          _lane3 = pickArbitraryLane((TransitionShortLanes | TransitionLongLanes) & ~wipLanes);
...
function findTransitionLane(lanePriority, wipLanes, pendingLanes) {
  ...
  {
    {
      throw Error( "Invalid transition priority: " + lanePriority + ". This is a bug in React." );
    }
  }
} // To ensure consistency across multiple updates in the same event, this should
// be pure function, so that it always returns the same lane for given inputs.
...
function createEventListenerWrapperWithPriority(targetContainer, domEventName, eventSystemFlags, priority) {
  var eventPriority = priority === undefined ? getEventPriorityForPluginSystem(domEventName) : priority;
  var listenerWrapper;
...
function removeTrappedEventListener(targetContainer, domEventName, capture, listener) {
...
function ensureListeningTo(rootContainerInstance, reactPropEvent, targetElement) {
  // If we have a comment node, then use the parent node,
  // which should be an element.
  var rootContainerElement = rootContainerInstance.nodeType === COMMENT_NODE ? rootContainerInstance.parentNode : rootContainerInstance; // Containers should only ever be element nodes. We do not
...
function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
  ...
  switch (tag) {
    ...
    case 'input':
      ...
      listenToNonDelegatedEvent('invalid', domElement); // For controlled components we always need to ensure we're listening
...
var getComponentStackByFiber = function getComponentStackByFiberDisabled(fiber) {
  return '';
};
...
function markComponentSuspended(fiber, wakeable) {
  {
    if (supportsUserTiming) {
      ...
      var componentStack = getComponentStackByFiber();
      performance.mark("--suspense-suspend-" + id + "-" + componentName + "-" + componentStack);
      wakeable.then(function () {
        return performance.mark("--suspense-resolved-" + id + "-" + componentName + "-" + componentStack);
      }, function () {
        return performance.mark("--suspense-rejected-" + id + "-" + componentName + "-" + componentStack);
      });
    }
  }
}
...
function markForceUpdateScheduled(fiber, lane) {
  {
    if (supportsUserTiming) {
      var componentName = getComponentName(fiber.type) || 'Unknown';
      var componentStack = getComponentStackByFiber();
      performance.mark("--schedule-forced-update-" + formatLanes(lane) + "-" + componentName + "-" + componentStack);
    }
  }
}
function markStateUpdateScheduled(fiber, lane) {
  {
    if (supportsUserTiming) {
      var componentName = getComponentName(fiber.type) || 'Unknown';
      var componentStack = getComponentStackByFiber();
      performance.mark("--schedule-state-update-" + formatLanes(lane) + "-" + componentName + "-" + componentStack);
    }
  }
}
  ...
  ReactStrictModeWarnings.flushLegacyContextWarning = function () {
    pendingLegacyContextWarning.forEach(function (fiberArray, strictRoot) {
      ...
      try {
        ...
        error('Legacy context API has been detected within a strict-mode tree.' + '\n\nThe old API will be supported in all 16.x releases, but applications ' + 'using it should migrate to the new version.' + '\n\nPlease update the following components: %s' + '\n\nLearn more about this warning here: https://fb.me/react-legacy-context', sortedNames);
      } finally {
        resetCurrentFiber();
      }
    });
      ...
      while (dependency !== null) {
        ...
        if (dependency.context === context && (dependency.observedBits & changedBits) !== 0) {
          ...
          if (fiber.tag === ClassComponent) {
            ...
            var update = createUpdate(NoTimestamp, pickArbitraryLane(renderLanes), null);
  ...
  if (mixedRef !== null && typeof mixedRef !== 'function' && typeof mixedRef !== 'object') {
    {
      ...
      !(element._owner && element._self && element._owner.stateNode !== element._self)) {
        ...
        if (!didWarnAboutStringRefs[componentName]) {
          {
            error('A string ref, "%s", has been found within a strict mode tree. ' + 'String refs are a source of potential bugs and should be avoided. ' + 'We recommend using useRef() or createRef() instead. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-string-ref', mixedRef);
          }
...
function updateMemoComponent(current, workInProgress, Component, nextProps, updateLanes, renderLanes) {
  if (current === null) {
    ...
    var child = createFiberFromTypeAndProps(Component.type, null, nextProps, null, workInProgress.mode, renderLanes);
      ...
      // Special path for hydration
      {
        ...
        if (_dehydrated !== null) {
          if (!didSuspend) {
            ...
            var fallbackChildFragment = mountSuspenseFallbackAfterRetryWithoutHydrating(current, workInProgress, _nextPrimaryChildren2, _nextFallbackChildren, renderLanes);
            ...
            if (state !== null) {
              {
                if (state.dehydrated !== null) {
                  ...
                  workInProgress.effectTag |= DidCapture; // We should never render the children of a dehydrated boundary until we
    ...
    case SuspenseListComponent:
      {
        ...
        if (renderedTail === null) {
          ...
          if (!didSuspendAlready) {
            ...
            // the expiration.
            now() * 2 - renderState.renderingStartTime > renderState.tailExpiration && renderLanes !== OffscreenLane) {
...
function requestUpdateLane(fiber, suspenseConfig) {
  ...
  if (suspenseConfig !== null) {
    ...
    var transitionLanePriority = timeoutMs === undefined || (timeoutMs | 0) < 10000 ? TransitionShortLanePriority : TransitionLongLanePriority;

    if (currentEventPendingLanes !== NoLanes) {
      currentEventPendingLanes = mostRecentlyUpdatedRoot !== null ? mostRecentlyUpdatedRoot.pendingLanes : NoLanes;
    }

    return findTransitionLane(transitionLanePriority, currentEventWipLanes, currentEventPendingLanes);
    ...
    case RootCompleted:
      {
        ...
        if ( // do not delay if we're inside an act() scope
        !shouldForceFlushFallbacksInDEV() && _mostRecentEventTime !== NoTimestamp && workInProgressRootCanSuspendUsingConfig !== null) {
          // If we have exceeded the minimum loading delay, which probably
          // means we have shown a spinner already, we might have to suspend
          // a bit longer to ensure that the spinner is shown for
          // enough time.
          var _msUntilTimeout2 = computeMsUntilSuspenseLoadingDelay(_mostRecentEventTime, workInProgressRootCanSuspendUsingConfig);
...
function discreteUpdates$1(fn, a, b, c, d) {
  ...
  try {
    setCurrentUpdateLanePriority(InputDiscreteLanePriority); // Should this
...
function computeMsUntilSuspenseLoadingDelay(mostRecentEventTime, suspenseConfig) {
      ...
      if (!didWarnAboutFindNodeInStrictMode[componentName]) {
        ...
        try {
          ...
          if (fiber.mode & StrictMode) {
            error('%s is deprecated in StrictMode. ' + '%s was passed an instance of %s which is inside StrictMode. ' + 'Instead, add a ref directly to the element you want to reference. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-find-node', methodName, methodName, componentName);
          } else {
            error('%s is deprecated in StrictMode. ' + '%s was passed an instance of %s which renders StrictMode children. ' + 'Instead, add a ref directly to the element you want to reference. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-find-node', methodName, methodName, componentName);
          }
        } finally {
...
function attemptHydrationAtCurrentPriority$1(fiber) {
  ...
  var eventTime = requestEventTime();
  var lane = requestUpdateLane(fiber, null);
  scheduleUpdateOnFiber(fiber, lane, eventTime);
  markRetryLaneIfNotHydrated(fiber, lane);
}
...
function createRootImpl(container, tag, options) {
  ...
  if (hydrate && tag !== LegacyRoot) {
    var doc = containerNodeType === DOCUMENT_NODE ? container : container.ownerDocument; // We need to cast this because Flow doesn't work
