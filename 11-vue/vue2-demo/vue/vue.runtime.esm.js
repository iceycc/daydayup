...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    try {
      ...
      // 所以render的第一个参数是 h ： vm.$createElement
...
function initData (vm) {
  var data = vm.$options.data; // 推荐data为函数 
...
function initGlobalAPI (Vue) {
  ...
  Vue.util = {
    ...
    defineReactive: defineReactive // Object.defineProperty
  };
  ...
  initMixin$1(Vue); // 定义 Vue.mixin 混合 将传人大属性混合到this.options中
...
function createPatchFunction (backend) {
  ...
  function removeNode (el) {
    var parent = nodeOps.parentNode(el); // 获取父节点
